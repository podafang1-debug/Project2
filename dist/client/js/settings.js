...
function renderSetting(c){
  ...
  if(p.accounts){
    html+='<div class="card"><b>正式账号治理</b><p style="margin:6px 0;color:var(--sub);font-size:14px">账号以手机号码为主键。专业账号审核、重置密码、撤销会话与儿童授权均在“账号管理”模块完成，并要求当前康复专业人员再次验证密码。</p></div>';
  }else{
    const phone=BACKEND_API.user?.phone||'';
    html+='<div class="card"><b>当前账号</b><p style="margin:6px 0 0;color:var(--sub);font-size:14px">'+esc(phone)+' · 可从登录页选择该账号支持的身份进入。</p></div>';
  }
