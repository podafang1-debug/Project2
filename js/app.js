...
function renderToday(){
  ...
  if(!todo.length){
    const emptyText=currentRole==="parent"?"今天没有待陪练任务，可以查看家庭任务或稍后再来":
  ...
  list.innerHTML=todo.map(t=>{
    ...
    const modName={attention:"注意力",memory:"记忆力",logic:"逻辑力"}[t.module];
    const color={attention:"#4F86F7",memory:"#10B981",logic:"#8B5CF6"}[t.module];
    return '<div class="task'+(over?" over":"")+'">'+
      '<div class="tic" style="background:'+color+'22">'+svg(t.module,color,26)+'</div>'+
      '<div class="meta"><b>'+esc(child?child.name:"未知")+' · '+modName+'</b>'+
  ...
  $$("#todayList .go-btn").forEach(b=>b.onclick=()=>{
    ...
    if(!PERMS[currentRole].train){toast("当前身份无训练权限");return;}
    openTrainer(t.childId,t.module,t.difficulty,()=>{t.done=true;saveAll();renderToday();});
  });
}
