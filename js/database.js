...
function createDemoDatabase(){
  ...
  const roleCodes={
    ...
    teacher:[DB_ACTIONS.TRAIN,DB_ACTIONS.PROGRESS_AUTH,DB_ACTIONS.PROFILE_EDIT,DB_ACTIONS.ASSESS_EDIT,DB_ACTIONS.AI_REVIEW,DB_ACTIONS.CARE_WRITE,DB_ACTIONS.CARE_SIGN,DB_ACTIONS.RISK_FLAG,DB_ACTIONS.RISK_RESOLVE,DB_ACTIONS.CONTENT_SUGGEST,DB_ACTIONS.AUDIT_LIMITED],
    // 复合管理员承接内容审核与机构治理，但不因此获得儿童临床档案读取权。
    admin:[DB_ACTIONS.PROGRESS_ANON,DB_ACTIONS.CONTENT_PUBLISH,DB_ACTIONS.ORG_MANAGE,DB_ACTIONS.ACCOUNT_MANAGE,DB_ACTIONS.OPERATIONS_READ,DB_ACTIONS.CONSENT_GOVERN,DB_ACTIONS.SHARING_MANAGE,DB_ACTIONS.BACKUP_GOVERN,DB_ACTIONS.AUDIT_FULL]
  };
...
// 权限迁移：移除旧演示库曾授予管理员的儿童档案管理权，落实“脱敏汇总”边界。
{
  ...
  const forbiddenAdminPermissions=[DB_ACTIONS.PROFILE_MANAGE,DB_ACTIONS.BACKUP_LIMITED,DB_ACTIONS.BACKUP_FULL]
    .map(code=>relationalDb.permissions.find(x=>x.code===code)?.permissionId).filter(Boolean);
  if(adminRole)relationalDb.rolePermissions=relationalDb.rolePermissions.filter(x=>!(x.roleId===adminRole.roleId&&forbiddenAdminPermissions.includes(x.permissionId)));
  // 从旧六角色数据库迁移：医疗职责并入专业人员，内容审核权限并入复合管理员。
  ...
  if(teacherRole)teacherRole.name='康复医疗专业人员';
  ...
  Object.entries(desired).forEach(([roleCode,codes])=>{const role=relationalDb.roles.find(x=>x.code===roleCode);const allowed=new Set(codes.map(code=>ensurePermission(code).permissionId));relationalDb.rolePermissions=relationalDb.rolePermissions.filter(x=>x.roleId!==role.roleId||allowed.has(x.permissionId));allowed.forEach(permissionId=>{if(!relationalDb.rolePermissions.some(x=>x.roleId===role.roleId&&x.permissionId===permissionId))relationalDb.rolePermissions.push({rolePermissionId:'rp_'+roleCode+'_'+permissionId,roleId:role.roleId,permissionId});});});
}
...
/** 正式授权仅由服务端管理员建立，浏览器不得自行补建账号与儿童绑定。 */
