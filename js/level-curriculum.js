...
function runCurriculumQuestions(childId,level,questions){
  const sheet=$('#trainSheet'),bodyId='curriculumQuestionBody',runId='CRUN-'+uid();let index=0,results=[],difficulty=level.difficulty;
  sheet.innerHTML='<h3>第 '+level.order+' 关 · '+esc(level.title)+'<button class="x" id="closeCurriculumRun">×</button></h3><div class="curriculum-run-dots">'+questions.map((_q,i)=>'<i class="'+(i===0?'on':'')+'"></i>').join('')+'</div><div id="'+bodyId+'"></div>';
  $('#trainMask').classList.remove('hidden');$('#closeCurriculumRun').onclick=()=>{$('#trainMask').classList.add('hidden');renderTrain($('#tabContent'));};const body=$('#'+bodyId);
  function render(){
  ...
  function answer(q,picked,hints,repeats,reactionMs){
    ...
    body.innerHTML='<div class="agent-answer-celebration '+(correct?'ok':'try')+'"><div>'+(correct?'🌟':'🌱')+'</div><h2>'+(correct?'完成啦！':'没关系')+'</h2><button class="btn-primary" id="nextCurriculumQuestion">'+(index+1===questions.length?'看看结果':'下一个游戏')+'</button></div>';$('#nextCurriculumQuestion').onclick=()=>{index++;$$('.curriculum-run-dots i',sheet).forEach((dot,i)=>dot.classList.toggle('on',i<=index));render();};
  }
  function finish(){
    ...
    body.innerHTML='<div class="train-summary"><div class="big">'+correct+' / '+results.length+'</div><h2>'+(passed?'闯关成功！ 🎉':'再试一次也很棒 🌱')+'</h2><p>'+(passed?'下一关已经打开。':'这一关还在等你，慢慢来。')+'</p><button class="btn-primary" id="finishCurriculum">'+(passed?'去看新关卡':'回到地图')+'</button></div>';$('#finishCurriculum').onclick=()=>{$('#trainMask').classList.add('hidden');renderTrain($('#tabContent'));};
  }
  render();
}
