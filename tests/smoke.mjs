...
if (await page.locator('#closeTrainingGate').count()) throw new Error('Pending plan was unexpectedly blocked');
await page.click('#closeUnifiedTrain');
await page.click('[data-tab="ai"]');
await page.click('[data-review-result="accepted"]');
const quickReviewStatus = await page.evaluate(() => {const stored=enhancedState.planVersions;enhancedState.planVersions=stored.filter(item=>item.childId!==activeChild);const status=currentPlanReview(activeChild).status;enhancedState.planVersions=stored;return status;});
if (quickReviewStatus !== 'reviewed') throw new Error(`AI review must remain unsigned, got ${quickReviewStatus}`);
await page.click('[data-tab="train"]');
...
if (await page.locator('#btnExport:not(.hidden),#btnImport:not(.hidden)').count()) throw new Error('Admin must not access child-data backup controls');
if (await page.locator('#childStrip:not(.hidden)').count()) throw new Error('Admin must not receive identifiable child records');
